// API ROUTE: src/app/api/email/bienvenida/route.ts
...
export async function POST(req: NextRequest) {
  ...
  await resend.emails.send({
    ...
    to: email,
    subject: '¡Bienvenido a GISTO! Tus 20 minutos están listos',
    html: `
<!DOCTYPE html>
<html>
...
<div style="max-width:560px;margin:0 auto;padding:40px 24px">
  ...
  <img src="https://www.thegisto.com/isotipo.png" alt="GISTO" width="64" height="64" style="display:block;margin:0 auto" />
      ...
      <div style="background:rgba(0,168,232,0.06);border:1px solid rgba(0,168,232,0.14);border-radius:12px;padding:20px;margin-bottom:24px;text-align:left">
        <div style="font-size:11px;font-weight:700;color:rgba(240,246,252,0.4);letter-spacing:2px;text-transform:uppercase;margin-bottom:14px">CON TUS 20 MINUTOS PUEDES</div>
        ${['Subir una grabación de hasta 20 minutos','Recibir cápsulas de video por módulo','Obtener documentos Word con resúmenes','Descargar quizzes y glosario automáticos'].map(item => `
      ...
      <a href="https://app.thegisto.com/upload" style="display:inline-block;background:#00A8E8;color:#000;padding:14px 32px;border-radius:10px;font-weight:800;font-size:15px;text-decoration:none">
